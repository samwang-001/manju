...
**确认**：`projects/{项目名}/定调方案.md` 存在且内容完整 → 进入阶段2
...
task(subagent_name="director",
     ...
     审核分镜质量。输出审核报告到 projects/{项目名}/分镜审核报告.md。
...
task(subagent_name="director",
     prompt="Gate5终审：
     1. 读取分镜表和所有审核报告
     2. 用 ffmpeg 获取 final.mp4 的时长、分辨率信息
     3. 全面评估：视觉一致性、配音同步、字幕完整、BGM音量、整体观感
     4. 输出终审报告到 projects/{项目名}/终审报告.md
     5. 给出最终结论：交付 / 需修复 / 不可交付")
```
